...
export class GCOVDataImporter<T={}> extends Importer<T> {
  protected eventBarrier = new EventBarrier
  /**
      ...
      if(match) {
        ...
        if((gcovOrResult = buffer.get(testIndex)) != undefined) {
          if(type == 'gcov') {
            ...
            } as CoverageAndResult<Partial<T>>
            this.emit('data', data)
            eventBarrier.notify('data', data)
          } else {  // type == 'result'
            const data = {
   ...
   * (with `--json-format` flag) and their corresponding result files
   */
  public async *importAsIterator(pathPrefix: string) {
    const { eventBarrier } = this
    this.done = false
    this.doImport(pathPrefix).catch(err => eventBarrier.abort('data', err))
    while(!this.done) {
      yield (await eventBarrier.waitFor('data')) as CoverageAndResult<Partial<T>>
    }
  }
   ...
   * (with `--json-format` flag) and their corresponding result files
   */
  public async import(pathPrefix: string) {
    for await (const _ of this.importAsIterator(pathPrefix)) {}
  }
}
